...
def calc_player_velocities(team, smoothing=True, filter_='Savitzky-Golay', window=7, polyorder=1, maxspeed = 12):
    ...
    second_half_start_index = team.loc[team['Period'] == 2].index[0]
    ...
    for player in player_ids: # cycle through players individually
        ...
        if smoothing:
            if filter_=='Savitzky-Golay':
                ...
                vx.loc[:second_half_start_index] = signal.savgol_filter(vx.loc[:second_half_start_index],window_length=window,polyorder=polyorder)
                vy.loc[:second_half_start_index] = signal.savgol_filter(vy.loc[:second_half_start_index],window_length=window,polyorder=polyorder)        
                ...
                # calculate first half velocity
                vx.loc[:second_half_start_index] = np.convolve( vx.loc[:second_half_start_index] , ma_window, mode='same' ) 
                vy.loc[:second_half_start_index] = np.convolve( vy.loc[:second_half_start_index] , ma_window, mode='same' )      
                # calculate second half velocity
                vx.loc[second_half_start_index:] = np.convolve( vx.loc[second_half_start_index:] , ma_window, mode='same' ) 
                vy.loc[second_half_start_index:] = np.convolve( vy.loc[second_half_start_index:] , ma_window, mode='same' ) 

...
# Actually need to run the first bit of D4 to use these
from D4_Data_Entry_Summary import team_coloring
from D4_Data_Entry_Summary import event_color
import sys
...
def plot_frame( hometeam, awayteam, figax=None, team_colors=team_coloring, field_dimen = (106.0,68.0), include_player_velocities=False, Playermarkersize=10, PlayerAlpha=0.7, annotate=False, num_move_x = 2, num_move_y = 2):
    ...
    for team,color in zip( [hometeam,awayteam], team_colors) :
        ...
        if annotate:
            [ax.text(team[x]+num_move_x, team[y]+num_move_y, x.split('_')[1], fontsize=30, color='white') for x, y in zip(x_columns, y_columns) if not (np.isnan(team[x]) or np.isnan(team[y]))]
    # plot ball
    ax.plot( hometeam['ball_x'], hometeam['ball_y'], 'ko', markersize=6, alpha=1.0, linewidth=0)
    
    return fig,ax
    
def save_match_clip(hometeam,awayteam, fpath, fname='clip_test', figax=None, frames_per_second=25, team_colors= team_coloring, field_dimen = (106.0,68.0), include_player_velocities=False, Playermarkersize=10, PlayerAlpha=0.7, title='Your Title Here'):
    ...
    with writer.saving(fig, fname, 100):
        for i in index:
            ...
            ax.set_title(f'{title} - {timestring}')
...
def plot_events( events, figax=None, field_dimen = (106.0,68), indicators = ['Marker','Arrow'], color=event_color, marker_style = 'o', alpha = 0.5, annotate=False):
    """ plot_events( events )

...
H_team_means = mean_position(H_tracking)
A_team_means = mean_position(A_tracking)
# Label deepest players
H_team_means = label_defenders(H_team_means, 'home')
A_team_means = label_defenders(A_team_means, 'away')
# Add labels to events
H_events = identify_defender_passing(H_events, H_team_means, "home")
A_events = identify_defender_passing(A_events, A_team_means, "away")
...
H_defender_pass_indexes = H_events[H_events['DefenderPass'] == 1].index.tolist()
H_defender_pass_indexes = A_events[A_events['DefenderPass'] == 1].index.tolist()
# Get build-ups sequences of each team
H_build_ups = create_build_sequences(H_events, H_tracking, 'home')
A_build_ups = create_build_sequences(A_events, A_tracking, 'away')



# Use Metrica functions to find goalkeeper
H_keeper = mio.find_goalkeeper(H_tracking)
A_keeper = mio.find_goalkeeper(A_tracking)
# Locate the frames where the GK passes the ball
H_gk_pass = locate_gk_pass_frames(H_events, str(H_keeper))
A_gk_pass = locate_gk_pass_frames(A_events, str(A_keeper))
# Filter GK's passes from H_events and create sequences
H_gk_passes = H_events[H_events['Start Frame'].isin(H_gk_pass['pass_frame'])]
A_gk_passes = A_events[A_events['Start Frame'].isin(A_gk_pass['pass_frame'])]



# Locate the frames where passes precede shots
H_pre_shot_pass = locate_pre_shot_pass_frames(H_events)
A_pre_shot_pass = locate_pre_shot_pass_frames(A_events)
# Filter the passes that precede shots from H_events and A_events, create sequences
H_passes_shots = H_events[H_events['Start Frame'].isin(H_pre_shot_pass['pass_frame'])]
A_passes_shots = A_events[A_events['Start Frame'].isin(A_pre_shot_pass['pass_frame'])]
...
H_possess = create_possession_sequences(H_events)
A_possess = create_possession_sequences(A_events)

...
# DATADIR = 'YOUR/FILE/PATH/HERE/sample-data-master/data'
...
# Get all shots
H_shots_events = H_events[H_events.Type=='SHOT']
A_shots_events = A_events[A_events.Type=='SHOT']
# Get all goals
H_goals_events = H_shots_events[H_shots_events['Subtype'].str.contains('-GOAL')].copy()
A_goals_events = A_shots_events[A_shots_events['Subtype'].str.contains('-GOAL')].copy()
# Add a column event 'Minute' to the data frame
H_goals_events['Minute'] = H_goals_events['Start Time [s]']/60.
A_goals_events['Minute'] = A_goals_events['Start Time [s]']/60.
...
def create_physical_summary(tracking_data, team_prefix):
...
H_physical_summary = create_physical_summary(H_tracking, 'Home')
A_physical_summary = create_physical_summary(A_tracking, 'Away')
